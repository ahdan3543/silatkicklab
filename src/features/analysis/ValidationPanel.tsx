...
export const ValidationPanel: React.FC<ValidationPanelProps> = ({ qualityReport }) => {
  ...
  const isReady = qualityReport.overallStatus === 'READY';
  const isWarning = qualityReport.overallStatus === 'WARNING';
  const isIncomplete = qualityReport.overallStatus === 'INCOMPLETE';
  ...
  return (
    <Card className="border-t-4 border-t-primary">
      ...
      {/* Progress Kelengkapan & Pesan Kesiapan */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-3 bg-slate-50 rounded-lg border border-dark-border text-xs">
        ...
        <div className="text-right w-full sm:w-auto">
          {qualityReport.isReadyForReport ? (
            ...
            <span className="inline-flex items-center gap-1.5 text-amber-700 font-semibold bg-amber-50 px-3 py-1.5 rounded-md border border-amber-200">
